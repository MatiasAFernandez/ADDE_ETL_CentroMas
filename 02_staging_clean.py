...
def asegurar_idempotencia(master_uri):
    """
    MODIFICADO: Ya no borra la base de datos completa.
    Solo asegura que la base de datos exista y limpia las tablas 'clean_'.
    """
    print(f"\n[*] [IDEMPOTENCIA] Asegurando integridad de '{CLEAN_DB_NAME}'...")
    engine_master = create_engine(master_uri, isolation_level="AUTOCOMMIT")
    
    with engine_master.connect() as conn:
        # 1. Crear la DB si no existe (sin borrarla)
        ...
        if not check_db:
            ...
            print(f"    -> Base de datos '{CLEAN_DB_NAME}' creada por primera vez.")
    ...
    print("    -> Entorno de Staging Clean preparado (Mapeos preservados).")

def inicializar_lookups(engine_clean):
    # (El mismo diccionario de la versión anterior)
...
def ejecutar_staging_clean():
    try:
        ...
        # Usamos inspector para ver qué tablas vinieron en el incremento
        inspector = inspect(engine_stg)
        stg_tables = inspector.get_table_names()
        print(f"[*] Tablas detectadas en el incremento actual: {stg_tables}")
        ...
        df_prod.to_sql('clean_products', engine_clean, if_exists='replace', index=False, dtype={'list_price': types.Numeric(12,2), 'fecha_carga': types.DateTime})

        # --- 2. SUCURSALES ---
        print("-> [2/5] Procesando Sucursales...")
        if 'stg_stores' in stg_tables:
        ...
        df_stores.to_sql('clean_stores', engine_clean, if_exists='replace', index=False, dtype={'opening_date': types.Date, 'surface_m2': types.Numeric(10,2), 'fecha_carga': types.DateTime})

        # --- 3. CLIENTES ---
        print("-> [3/5] Procesando Clientes...")
        df_cust_clean = pd.DataFrame()
        df_lkp_duplicados = pd.DataFrame(columns=['customer_id', 'id_sobreviviente'])

        if 'stg_customers' in stg_tables:
            ...
            df_lkp_duplicados = df_cust[['customer_id', 'id_sobreviviente']].copy()
            ...
            print("    [-] No hay clientes nuevos. Generando estructura vacía.")
            df_cust_clean = pd.DataFrame(columns=['customer_id', 'customer_code', 'customer_name', 'gender', 'birth_date', 'city', 'province', 'registration_date'])

        # --- 4. VENTAS ---
        print("-> [4/5] Procesando Ventas...")
        if 'stg_orders' in stg_tables:
            df_orders = pd.read_sql("SELECT order_id, order_date, customer_id, store_id, net_amount FROM stg_orders", engine_stg)
            df_orders['order_date'] = pd.to_datetime(df_orders['order_date'])
            
            # Reasignar clientes SI hay datos en el lookup de duplicados
            if not df_lkp_duplicados.empty:
                df_orders = df_orders.merge(df_lkp_duplicados, on='customer_id', how='left')
                df_orders['customer_id'] = df_orders['id_sobreviviente'].fillna(df_orders['customer_id'])
                df_orders = df_orders.drop(columns=['id_sobreviviente'])

            # Corregir fecha_registro de clientes SI entraron clientes en este incremento
            if not df_cust_clean.empty:
                ...
                # Solo aplicar regla donde la fecha_min_compra no sea nula (clientes con compras en este delta)
                mask_fecha = (df_cust_clean['registration_date'] > df_cust_clean['fecha_min_compra']) & df_cust_clean['fecha_min_compra'].notnull()
                df_cust_clean.loc[mask_fecha, 'registration_date'] = df_cust_clean['fecha_min_compra']
        ...
        # --- GUARDAR CLIENTES Y VENTAS ---
        if not df_cust_clean.empty: df_cust_clean['fecha_carga'] = fecha_ejecucion
        df_cust_clean.to_sql('clean_customers', engine_clean, if_exists='replace', index=False, dtype={'birth_date': types.Date, 'registration_date': types.Date, 'fecha_carga': types.DateTime})
        df_lkp_duplicados.to_sql('lkp_duplicados', engine_clean, if_exists='replace', index=False)
        
        df_orders.to_sql('clean_orders', engine_clean, if_exists='replace', index=False, dtype={'order_date': types.Date, 'net_amount': types.Numeric(12,2), 'fecha_carga': types.DateTime})

        # --- 5. DETALLES ---
        print("-> [5/5] Procesando Detalles...")
        if 'stg_order_details' in stg_tables:
            ...
            if 'stg_products' in stg_tables:
                ...
                df_det = df_det.merge(df_p, on='product_id', how='left')
                ...
                print("    [!] 'stg_products' no recibida. Se omite redondeo de unidades para no afectar métricas.")
            
            df_det['fecha_carga'] = fecha_ejecucion
        else:
            print("    [-] No hay detalles de ventas nuevos. Generando estructura vacía.")
